...
### 1.1（2026-09-04 に審査提出。投げ銭と同時提出）

- 投げ銭「開発者にコーヒーを奢る」（消耗型・¥200・`com.zzzjjj080.HealthExport.coffee`）
  - 置き場所は2つ。詳しい設定の「形式」タブ最下部と、書き出した直後の1行
  - 審査用スクリーンショットは `store/iap/coffee-settings.png`（UIテストから撮った）
  - 実機のSandboxで購入まで確認済み
- 不具合の報告・要望を送るボタン
